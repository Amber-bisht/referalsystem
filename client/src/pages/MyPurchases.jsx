        ...
        <div className="container mx-auto px-4 py-8 max-w-5xl">
            ...
            {history.length === 0 ? (
                <div className="text-center py-24 bg-white border border-slate-100 rounded-2xl shadow-sm">
                    ...
                    <h2 className="text-xl font-bold text-slate-900 mb-2">No purchases yet</h2>
                    <p className="text-sm text-slate-500 mb-8 max-w-xs mx-auto">Purchase your first gift card to generate a unique voucher code and start earning.</p>
                ...
                <div className="space-y-4">
                    {history.slice().reverse().map((order, index) => (
                        <div key={index} className="bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-sm p-6">
                            <div className="flex flex-col md:flex-row items-center gap-6">
                                ...
                                <div className="flex-1 text-center md:text-left">
                                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 mb-1 justify-center md:justify-start">
                                        <h3 className="text-lg font-bold text-slate-900 leading-tight">{order.productName}</h3>
                                        <span className="inline-flex px-2 py-0.5 bg-slate-100 text-slate-500 text-[9px] font-bold uppercase tracking-widest rounded-md w-fit mx-auto md:mx-0">Voucher</span>
                                    </div>
                                    <div className="flex items-center justify-center md:justify-start gap-2">
                                        ...
                                        <span className="text-[10px] font-bold text-slate-900 uppercase">₹{order.price.toLocaleString()}</span>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center justify-center">
                                    <span className="px-4 py-1.5 bg-slate-900 text-white text-[10px] font-bold uppercase tracking-widest rounded-lg shadow-sm">
                                        Success
                                    </span>
                                </div>
                            </div>
